...
var assignProperty = require("../helpers/assignProperty"),
    ...
    string_split = StringPrototype.split,// Native, don't modify
    compliantExecNpcg;
  ...
  module.exports = function (separator, limit) {
    ...
    while ((match = separator.exec(string))) {
      ...
      if (lastIndex > lastLastIndex) {
        ...
        if (!compliantExecNpcg && 1 < match.length) {
          match[0].replace(separator2, matchReplacer);
        }
        if (1 < match.length && match.index < string.length) {
